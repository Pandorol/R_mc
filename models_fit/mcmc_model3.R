POW_MSE <-function(params,y_real,x_real)
{
	...
	y_pred = params[1]*exp(-params[2]*x_real+params[3])+params[4]
...
params_aic<-c(0,0,0,0,0)
names(params_aic)<-c("a","b","c","d","aic")
...
for(i in 1:50000)
{
	proposal<-current+rnorm(4,0,0.05)
...
y_pred = current[1]*exp(-current[2]*c(0:max(x_real))+current[3])+current[4]

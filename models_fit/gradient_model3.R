POW_MSE <-function(params,y_real,x_real)
{
	...
	y_pred = params["a"]*exp(-params["b"]*x_real+params["c"])+params["d"]
...
POW_MSE_PLEAD <-function(params,y_real,x_real)
{
	...
	expPOW = expression((a*exp(-b*x+c)+d-y)^2)
	PLEAD<-c(sum(eval(D(expPOW,"a")))/length(x),sum(eval(D(expPOW,"b")))
	/length(x),sum(eval(D(expPOW,"c")))/length(x),sum(eval(D(expPOW,"d")))/length(x))
	names(PLEAD) = c("a","b","c","d")
	return(PLEAD)
}
...
params = c(1,1,1,0)#初始值
names(params) = c("a","b","c","d")
params_out = POW_MSE_OPTIM(params,y_real=y_real,x_real=x_real,1e-4)
y_pred = params_out["a"]*exp(-params_out["b"]*c(0:max(x_real))+params_out["c"])+params_out["d"]
